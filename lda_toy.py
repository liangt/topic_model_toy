    ...
    for ite in range(max_iter):
        ...
        for m, doc in enumerate(docs_words_topic):
            for n, topic in enumerate(doc):
                ...
                p = (topics_words[:, word]+beta)/(topics_words_sum+num_words*beta)*\
                    (docs_topics[m, :]+alpha)/(docs_topics_sum[m]+num_topics*alpha)
                p /= p.sum()
        ...
        if ite > burn_in and lag > 0 and ite % lag == 0:
            theta += (docs_topics + alpha) / (docs_topics_sum[:, np.newaxis] + num_topics * alpha)
            phi += (topics_words + beta) / (topics_words_sum[:, np.newaxis] + num_words * beta)
            num_stats += 1
...
if __name__ == '__main__':
    ...
    theta, phi = lda(docs_words, len(dictionary), 15, max_iter=50, burn_in=30, lag=5)
